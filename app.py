...
def preprocess_closed_issue_data():
    ...
    cursor.execute(
        "SELECT * FROM GITHUB_ISSUES.PUBLIC.ISSUES WHERE STATE='closed' ORDER BY ISSUE_NUMBER"
    )
...
def preprocess_open_issue_data():
    ...
    # Execute query to select all rows from the ISSUES table
    cursor.execute(
        "SELECT * FROM GITHUB_ISSUES.PUBLIC.ISSUES WHERE STATE='open' ORDER BY ISSUE_NUMBER"
    )
...
def compute_similarity_faiss(
    tokenized_issue_data, tokenized_issue_text, max_chunk_size=512
):
    ...
    if new_embedding.shape != (768,) or any(
        embedding.shape != (768,) for embedding in embeddings
    ):
    ...
    issue_embeddings_list = {
        issue_number: ", ".join(map(str, embedding.tolist()))
        for issue_number, embedding in zip(issue_numbers, embeddings)
    }
    ...
    if get_issues_button:
        ...
        if issue_text:
            ...
            with st.spinner("Computing Similarity Scores..."):
                issue_embeddings, issue_similarity_list = compute_similarity_faiss(
                    tokenized_texts, tokenized_issue_text
                )
                similar_issues = [
                    (issue_number, similarity_percentage)
                    for issue_number, similarity_percentage in issue_similarity_list
                    if similarity_percentage > 98
                ]
                for issue_number, similarity_percentage in similar_issues:
                    issue_url = get_issue_url(issue_number)
                    st.write(
                        f"Issue Number {issue_number}: {similarity_percentage:.2f}% - {issue_url}"
                    )
                if not similar_issues:
                    with st.spinner(
                        "No similar closed issues found. Checking open issues..."
                    ):
                        (
                            issue_embeddings,
                            issue_similarity_list_open,
                        ) = compute_similarity_faiss(
                            tokenized_texts_open, tokenized_issue_text
                        )
                        similar_issues_open = [
                            (issue_number, similarity_percentage)
                            for issue_number, similarity_percentage in issue_similarity_list_open
                            if similarity_percentage > 90
                        ]
                        for issue_number, similarity_percentage in similar_issues_open:
                            issue_url = get_issue_url(issue_number)
                            print(issue_number, issue_embeddings)
                            st.write(
                                f"Issue Number {issue_number}: {similarity_percentage:.2f}% - {issue_url}"
                            )
